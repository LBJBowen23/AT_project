...
% Acctually, this y axis is normalized total distance within cluster,
% rather than vriance explained
% figure;
D_norm = D ./ max(D).*100;
plot(D_norm,'b*--','linewidth',2);
xlim([1,max(ToTest)]);
xticks(1:max(ToTest));
xlabel('Number of Clusters');
ylabel('Percentage of Variance Explained');
hold on;
plot(4,[D_norm(4)],'ro','MarkerSize',16,'linewidth',2);
set(gca,'ytick',[0:10:100],'yticklabel',[0:10:100]);
% title('Cluster Number of K');
set(gca,'FontSize',16,'Fontname', 'Calibri');


Var=D(1:end-1)-D(2:end); %calculate %variance explained
PC=cumsum(Var)/(D(1)-D(end));
% plot the variance explained by number of cluster
% figure
%plot(PC,'b*--');
%ylabel('Percentage of variance explained');
